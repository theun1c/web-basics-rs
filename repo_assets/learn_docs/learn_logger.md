...
Время в логгере спокойно пофиксилось с помощью пакета chrono

На счет приватности log. Сейчас я пишу весь код в main.rs и поэтому имею доступ ко всем функциям внутри файла. Если я вынесу логгер в отдельный файл (а так стоит сделать), то у меня log будет недоступен. 

В Rust приватность работает на уровне модуля, а не на уровне структуры.